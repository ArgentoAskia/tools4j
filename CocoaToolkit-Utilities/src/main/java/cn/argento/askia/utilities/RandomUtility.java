...
//        }
        ...
        System.out.println(randomUnsignedInt());
    }
     ...
     * 如 {@code RandomUtils.randomInt(3, 5)}则可能生成的数字范围是: [3, 4, 5]
     *
     * @param low 生成范围下限
     * @param high 生成范围上限
     * @return 范围内的整型随机数
     ...
     * 如：{@code RandomUtils.randomInt(3, 8, false, false)}则生成的数字范围在：[4, 5, 6, 7]
     *
     * @param low 生成范围下限
     * @param high 生成范围上限
     ...
     * @return 范围内的整型随机数
     * @since 1.4
     * @see RandomUtility#randomInt(int, int)
     */
     ...
     *
     * <p>
     * <strong>生成方法参考 {@link RandomUtility#randomShort()}</strong>
     *
     * @return 单字节范围的随机数
     * @see RandomUtility#randomShort()
     * @since 1.0
     */
    ...
    public static <T> List<T> randomList(int listMinSize, int listMaxSize, Supplier<T> randomTypeCreator){
        final int randomSize = randomInt(listMinSize, listMaxSize);
        List<T> result = new ArrayList<>();
        for (int j = 0; j < randomSize; j++) {
            final T randomT = randomTypeCreator.get();
            result.add(randomT);
        }
        return result;
    }

    // TODO: 2024/2/22  method is not support array
    @Deprecated
     ...
     * @return 英文字母字符数组
     * @since 1.4
     * @see RandomUtility#randomAlphabets(int, boolean)
     */
     ...
     * @return 英文字母字符数组
     * @since 1.4
     * @see RandomUtility#randomAlphabets()
     */
     ...
     * @return 英文字母+数字字符数组
     * @since 1.4
     * @see RandomUtility#randomAlphabetsNumbers()
     */
     ...
     * @return 英文字母+数字字符数组
     * @since 1.4
     * @see RandomUtility#randomAlphabetsNumbers(int, boolean)
     */
    ...
    // TODO: 2023/3/6 randomColor() impl
    public static Color randomRGBColor(){
        final int r = randomUnsignedByte();
        final int g = randomUnsignedByte();
        final int b = randomUnsignedByte();
        return new Color(r, g, b);
    }

    // TODO: 2024/2/22 randomColor() impl
    public static Color randomRGBAColor(){
        final int r = randomUnsignedByte();
        final int g = randomUnsignedByte();
        final int b = randomUnsignedByte();
        final int a = randomUnsignedByte();
     ...
     * @param noRepeat
     * @return
     * @throws StringIndexOutOfBoundsException 当noRepeat为true,且baseString的长度大于length时抛出这个异常
     * @since 1.5
     */
    public static String randomString(String baseString, int length, boolean noRepeat){
        if (noRepeat && baseString.length() < length){
            throw new StringIndexOutOfBoundsException("不重复的选择的情况下,length参数要小于等于baseString字符串的长度! 请检查!");
        }
    ...
    public static long randomUnsignedInt(){
        ...
        // 使用高32位为1，第三十二位为0做结果铺垫
        long result = 0xFFFFFFFFL << 32;
        // 相当于result + i，第32位变成i
        result = (result | i);
        // 右移32位，让高32位全部为0,低32位全部为1
        long a = (result >>> 32);
        // 做且操作，保证高32位为0,低32位就是i本身，结果就是Unsigned int的范围
        result = result & a;
        return result;
    }

    public static int randomUnsignedShort(){
        return random.nextInt(MAX_UNSIGNED_SHORT + 1);
    }
    ...
    public static int randomUnsignedByte(){
        int i = random.nextInt(MAX_UNSIGNED_BYTE + 1);
        return i;
    }
