...
public class IOStreamUtility2 {
     ...
     * @return 返回读取的所有字节
     * @since 1.0
     * @see IOStreamUtility#readAllBytes(InputStream, int)
     */
     ...
     * @return 返回读取的所有字节
     * @since 1.0
     * @see IOStreamUtility#peekBytes(InputStream, int, byte[])
     */
     ...
     * @param bufferSize 每次读取数据所使用的buffer的大小（每次能从流里面读取多少个字节），可以使用{@link IOStreamUtility}的{@code BUFFER_X_SIZE}常量
     * @param data peek的数据，用户需要peek多少数据则创建多大的数组即可
     * @return 一个新的输入流，是第一个参数的输入流的拷贝
     * @since 1.0
     * @see IOStreamUtility#peekBytes(InputStream, byte[])
     */
