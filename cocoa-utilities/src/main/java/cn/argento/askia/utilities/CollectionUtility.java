...
public class CollectionUtility {
    ...
    /**
     * 增强类型的{@link Map#toString()}, 原版本的{@link Map#toString()}在遇到数组的时候只会打印出数组的引用, 此版本会对Map的value进行判断,
     * 如果发现他是数组的话则会直接使用{@link Arrays#toString(Object[])}进行格式化后再拼接。
     *
     * @param map
     * @return
     */
    public static String toString(Map<?, ?> map){
        ...
        for (Map.Entry<?, ?> entry : entries){
            ...
            if (value.getClass().isArray()){
                ...
                final String s = Arrays.toString(all);
                keyValue += s;
            }
            else{
    ...
    public static void main(String[] args) {
        ...
        list2.add(4);
        list2.add(5);
        System.out.println(equals(list, list2));
    }
}
